...
const addCommonOptions = (cmd) => {
    return cmd
        .option("-c, --credentials <path>", "Path to Google service account credentials JSON file", "./credentials.json")
...
const checkConfig = (options) => {
    if (!options.spreadsheet) {
        console.error("❌ Spreadsheet ID is required. Use -s option or set GOOGLE_SPREADSHEET_ID environment variable.");
        process.exit(1);
    }
    if (!fs.existsSync(options.credentials)) {
        console.error(`❌ Credentials file not found: ${options.credentials}`);
        console.error("Please download your Google Service Account key file and specify its path with -c option.");
        process.exit(1);
    }
    return {
...
addCommonOptions(program
    ...
    .option("--languages <langs>", "Comma-separated list of languages", "en,ko")).action(async (options) => {
    try {
...
program.on("--help", () => {
    ...
    console.log("  $ i18n-sheets init -s 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms");
...
program.parse();
//# sourceMappingURL=i18n-sheets.js.map